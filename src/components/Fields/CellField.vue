<template>
  <FieldLabelWrapper
    :label="label"
    :help-text="helpText ?? $t('message.Fields.Cell.HelpText')"
  ...
  >
    <input
      class="input"
      type="text"
      :placeholder="placeholder ?? $t('message.Fields.Cell.Placeholder')"
      v-model="value"
      @input="validate"
    />
  </FieldLabelWrapper>
...
export default {
  ...
  methods: {
    validate() {
      if (!this.optional && !this.value && this.format !== 'remaining') {
    ...
    store(builder: Builder): void {
      if (this.value) {
        const value = Cell.fromBase64(this.value);

        if (this.format == 'remaining') {
          builder.storeSlice(value.asSlice());
          return;
        }

        if (this.optional) {
          builder.storeMaybeRef(value);
          return;
        }
