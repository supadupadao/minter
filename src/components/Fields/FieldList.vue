<template>
  <div v-for="(input, index) in inputs" v-bind:key="`${formTitle}_${index}`">
    ...
    <UintField v-else-if="input.type == 'uint'" v-bind="input"
      :ref="el => registerElement(index, el as BaseFieldElement)" />
    <IntField v-else-if="input.type == 'int'" v-bind="input"
      :ref="el => registerElement(index, el as BaseFieldElement)" />
    <CellField v-else-if="input.type == 'cell'" v-bind="input"
      :ref="el => registerElement(index, el as BaseFieldElement)" />
    ...
    <BooleanField v-else-if="input.type == 'bool'" v-bind="input"
      :ref="el => registerElement(index, el as BaseFieldElement)" />
    <UnknownField v-else-if="input.type == 'any'" v-bind="input" label="Any field TODO" expected="any"
      :ref="el => registerElement(index, el as BaseFieldElement)" />
    <UnknownField v-else-if="input.type == 'empty'" v-bind="input" label="Empty field TODO" expected="empty"
      :ref="el => registerElement(index, el as BaseFieldElement)" />
    <UnknownField v-else-if="input.type == 'text'" v-bind="input" label="Text field TODO" expected="text"
      :ref="el => registerElement(index, el as BaseFieldElement)" />
...
export default {
  components: { StringField, CoinsField, UintField, IntField, CellField, SliceField, AddressField, BooleanField, UnknownField },
  props: {
    formTitle: {
      type: String,
      required: true,
