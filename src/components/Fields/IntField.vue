<template>
  <FieldLabelWrapper
    :label="label"
    :help-text="helpText ?? $t('message.Fields.Int.HelpText', { min, max, format })"
  ...
  >
    <input
      class="input"
      type="number"
      :placeholder="placeholder ?? $t('message.Fields.Int.Placeholder', { min, max, format })"
      v-model="value"
      @input="validate"
    />
  </FieldLabelWrapper>
...
export default {
  ...
  methods: {
    ...
    store(builder: Builder): void {
      if (this.optional) {
        builder.storeMaybeInt(this.value ? parseInt(this.value) : null, this.format);
      } else {
        builder.storeInt(parseInt(this.value), this.format);
      }
    },
  },
};
